...
namespace OnyxDoc.SubscriptionService.Application.SubscriptionPlanPricings.Commands
{
    ...
    public class UpdateSubscriptionPlanPricingStatusCommandHandler : IRequestHandler<UpdateSubscriptionPlanPricingStatusCommand, Result>
    {
        ...
        public async Task<Result> Handle(UpdateSubscriptionPlanPricingStatusCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                switch (request.Status)
                {
                    case Status.Inactive:
                        ...
                        message = "Subscription plan pricing is now inactive!";
                        break;
                    case Status.Active:
                        entity.Status = Status.Active;
                        message = "Subscription plan pricing was successfully activated!";
                        break;
                    case Status.Deactivated:
                        message = "Subscription plan pricing was deactivated!";
                        break;
                    default:
                        break;
                }
            ...
            {
                return Result.Failure($"Subscription plan pricing status update failed. Error: { ex?.Message + Environment.NewLine + ex?.InnerException?.Message}");
            }
        }
    }
}
