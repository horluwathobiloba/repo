...
namespace Onyx.ContractService.Application.Contracts.Commands.ExpiryEmailNotification
{
    public class GenericEmailNotificationCommand : IRequest<Result>
    {

    }

    public class GenericEmailNotificationCommandHandler : IRequestHandler<GenericEmailNotificationCommand, Result>
    {
        ...
        public GenericEmailNotificationCommandHandler(IApplicationDbContext context, IMapper mapper, IBlobStorageService blobService, IAuthService authService,
        ...
        public async Task<Result> Handle(GenericEmailNotificationCommand request, CancellationToken cancellationToken)
        {
            try
            {

                var contracts = await _context.Contracts.Where(a => a.Status == Status.Active).ToListAsync();
                ...
                foreach (var contract in contracts)
                {
                    ...
                    if (reminderConfiguration  != null && reminderConfiguration.Count > 0)
                    {
                        ...
                        if (result.result.Succeeded)
                        {
                            if (result.sendEmail)
                            {
                                var setupEmail = SetupExpiryEmailAndInbox(contract, approver, loginPage, _reminderScheduleService, user);
                ...
                //at the end of the day, deplete the recurring count balance
        ...
        private (Result result, EmailVm email, Inbox inbox) SetupExpiryEmailAndInbox(Domain.Entities.Contract contract, string approver, 
                                                                        string loginPage, IReminderScheduleService _reminderScheduleService, UserDto user)
        {
            ...
            try
            {
                ...
                if (approver != null)
                {
                    ...
                    {
                        Subject = "Contract Expiry for -" + contract.Name,
                        Text = emailMessage,
                                                             ...
                                                             string approver, UserDto userDetails, string emailMessage)
        {
            ...
            if (userDetails != null)
            {
                inbox = new Inbox
                {
                    Name = "Contract Expiry for -" + contract.Name,
