...
namespace OnyxDoc.FormBuilderService.Application.Documents.Commands
{
    ...
    public class DeleteSubscriptionCommandHandler : IRequestHandler<DeleteDocumentCommand, Result>
    {
        ...
        public async Task<Result> Handle(DeleteDocumentCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                return Result.Success("Document deleted successfully", entity);
            }
            catch (Exception ex)
            {
                return Result.Failure($"Delete document failed. Error: { ex?.Message + Environment.NewLine + ex?.InnerException?.Message}");
            }
