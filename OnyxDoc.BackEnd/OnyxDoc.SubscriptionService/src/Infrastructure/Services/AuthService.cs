...
namespace OnyxDoc.SubscriptionService.Infrastructure.Services
{
    ...
    public class AuthService : IAuthService
    {
        ...
        public AuthService(IConfiguration configuration, IRestClient client, IAPIClient apiClient)
        {
        ...
        public async Task<bool> ValidateSubscriberData(string authToken, int subscriberId, string userId, [Optional] int roleId, [Optional] int jobFunctionId)
        {
            ...
            var response = await this.GetSubscriberAsync(authToken, subscriberId, userId);

            var orgResponse = subscriberId <= 0 ? null : response;

            if (response != null)
            {
                this.Subscriber = response.Entity;
            }
        ...
        public async Task<EntityVm<SystemSettingDto>> GetSystemSettingsAsync(string authToken, int subscriberId, string userId, int systemSettingsId)
        {
            try
            {
                this.SetAuthToken(authToken);
