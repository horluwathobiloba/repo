...
namespace RubyReloaded.AuthService.Application.Common.Interfaces
{
    public class Base64ToFileConverter : IBase64ToFileConverter
    {
        ...
        public async Task<string> ConvertByteImageToFile(byte[] Image, string fileName)
        {
            var filePath = "";
            ...
            try
            {
                ...
                if (Image.Length > 0)
                {
                    ...
                    filePath = await _blobStorageService.UploadFileToBlobAsync(fileName, Image, "image/png");
                }
                return filePath;
            }
            catch (Exception ex)
        ...
        public async Task<string> ConvertByteToFile(byte[] uploadData, string fileName, string mimeType)
        {
            var filePath = "";
            ...
            try
            {
                ...
                var uploads = Path.Combine(directory, "videos");
                bool exists = Directory.Exists(uploads);
                if (!exists)
                    Directory.CreateDirectory(uploads);
                ...
                {
                    filePath = await _blobStorageService.UploadFileToBlobAsync(fileName, uploadData,mimeType);
                }
                return filePath;
            }
            catch (Exception ex)
