...
{
    public class GetUserByContractApproverPermissionsQuery : IRequest<Result>
    {
        public int OrganizationId { get; set; }
        public string UserId { get; set; }
    }

    public class GetUserByContractApproverPermissionsQueryHandler : IRequestHandler<GetUserByContractApproverPermissionsQuery, Result>
    {
        private readonly IApplicationDbContext _context;
        private readonly IIdentityService _identityService;
        private readonly IMapper _mapper;

        public GetUserByContractApproverPermissionsQueryHandler(IApplicationDbContext context, IIdentityService identityService, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        ...
        public async Task<Result> Handle(GetUserByContractApproverPermissionsQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var permissionName = Regex.Replace(PowerUsersPermissions.ApproveAndRejectContractRequest.ToString(), "([A-Z])", " $1").Trim();
                var permissions = await _context.RolePermissions.Where(a=>a.Permission == permissionName 
                                                                        && a.OrganizationId == request.OrganizationId
                                                                        && a.Status == Status.Active).Distinct().ToListAsync();
