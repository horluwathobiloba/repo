...
namespace OnyxDoc.SubscriptionService.API.Controllers
{
    ...
    public class SubscriptionPlanPricingsController : ApiController
    {
        ...
        [HttpPost("deletesubscriptionpricing")]
        public async Task<ActionResult<Result>> DeleteSubscriptionPlanPricing(DeleteSubscriptionPlanPricingCommand command)
        {
            try
            {
                command.AccessToken = accessToken;
            ...
            {
                return Result.Failure($"Delete subscription currency failed. Error: {ex?.Message ?? ex?.InnerException?.Message }");
            }
        }
