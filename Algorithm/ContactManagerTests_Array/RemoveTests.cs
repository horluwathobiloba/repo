...
namespace ContactManagerTests_Array
{
    [TestClass]
    public class RemoveTests
    {
        ...
        [TestMethod]
        public void RemoveRemovesWhenFull()
        {
            ContactStore store = new ContactStore();
            for (int i = 0; i < 100; i++)
            {
                Contact c = store.Add(Contact.Create("fn", "ln", "street", "city", "st", "zip"));
            }

            Assert.AreEqual(100, store.Contacts.Count());
            var toDelete = store.Contacts.First();

            store.Remove(toDelete);

            Assert.AreEqual(99, store.Contacts.Count());

            Assert.AreEqual(2, store.Contacts.First().ID);
            Assert.AreEqual(100, store.Contacts.Last().ID);
        }
