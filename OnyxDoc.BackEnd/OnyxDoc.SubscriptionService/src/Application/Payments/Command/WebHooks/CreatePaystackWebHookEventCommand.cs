...
public class CreatePaystackWebHookEventCommandHandler : IRequestHandler<CreatePaystackWebHookEventCommand,Result>
{
    ...
    public async Task<Result> Handle(CreatePaystackWebHookEventCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var result = JsonConvert.DeserializeObject<PaystackPaymentResponse>(request.Result);
            ...
            {
                return Result.Failure(new string[] { $"Update new Payment failed because a payment with the same payment intent id: {result.Data.Access_Code}  already exists. Please create a new order before proceeding to pay or contact support for more assistance." });
            }
