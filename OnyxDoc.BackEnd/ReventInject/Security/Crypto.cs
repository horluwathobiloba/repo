...
namespace ReventInject.Security
{
	...
	public class Crypto
	{
		...
		private static byte[] IV_24 = new byte[] {37, 28, 19, 44,
            25, 170, 122, 25, 25, 57, 127,	5, 22, 1, 66, 65, 14, 155,224, 64, 9, 77, 18, 251
		};
		...
		private static Algorithm _algorithm = default(Algorithm);
		...
		private static byte[] _Encrypt(byte[] Content)
		{
			...
			if (_algorithm == Algorithm.RSA) {
				...
				try {
					//Encrypt the string
					cipherBytes = SymmetricEncrypt(provider, Content, _key, (int)NumBytes);
				} catch (CryptographicException ex) {
		...
		private static byte[] _Decrypt(byte[] Content)
		{
			...
			if (_algorithm == Algorithm.RSA) {
				...
				try {
					clearBytes = SymmetricDecrypt(provider, encText, _key, (int)NumBytes);
				} catch (CryptographicException ex) {
					throw ex;
				} finally {
					//Free any resources held by the SymmetricAlgorithm provider
		...
		private static byte[] RSADecrypt(string encText)
		{
			...
			for (int blockIndex = 0; blockIndex <= blockCount - 1; blockIndex++) {
				...
				dynamic currentBlockHex = encText.Substring(blockIndex * (RSA_DECRYPTBLOCKSIZE * 2), RSA_DECRYPTBLOCKSIZE * 2);
