...
{
    class UpdatePGProductStatusValidator : AbstractValidator<UpdatePGProductStatusCommand>
    {
        public UpdatePGProductStatusValidator()
        {
            RuleFor(v => v.SubscriberId).GreaterThan(0).WithMessage("Subscriber must be specified!");
            RuleFor(v => v.SubscriptionPlanId).GreaterThan(0).WithMessage("Subscription plan must be specified!");
            RuleFor(v => v.Id).GreaterThan(0).WithMessage("Payment gateway product identifier must be specified!");
