...
{
    public class UpdateCurrencyStatusCommand : AuthToken, IRequest<Result>
    {
    ...
    public class UpdateCurrencyStatusCommandHandler : IRequestHandler<UpdateCurrencyStatusCommand, Result>
    {
        private readonly IApplicationDbContext _context;
        private readonly IMapper _mapper;
        private readonly IAuthService _authService;

        public UpdateCurrencyStatusCommandHandler(IApplicationDbContext context, IMapper mapper, IAuthService authService)
        {
            _context = context;
            _mapper = mapper;
        ...
        public async Task<Result> Handle(UpdateCurrencyStatusCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                switch (request.Status)
                {
                    ...
                    case Status.Deactivated:
                        message = "Currency was deactivated!";
                        break;
                    default:
                        break;
                }
            ...
            {
                return Result.Failure($"Currency status update failed. Error: { ex?.Message + Environment.NewLine + ex?.InnerException?.Message}");
            }
        }
    }
}
