...
{
    public class CreateDocumentPagesCommandValidator : AbstractValidator<CreateDocumentPagesCommand>
    {
        public CreateDocumentPagesCommandValidator()
        {
            RuleFor(v => v.SubscriberId).GreaterThan(0).WithMessage("Subscriber must be specified!");
            RuleFor(v => v.DocumentId).GreaterThan(0).WithMessage("Document id must be specified!");
            RuleFor(v => v.UserId).NotEmpty().WithMessage("User id must be specified!");
        }
    }

    public class CreateDocumentPageRequestValidator : AbstractValidator<CreateDocumentPageRequest>
    {
        public CreateDocumentPageRequestValidator()
        {
