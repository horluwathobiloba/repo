...
{
    public class GetUserBySignAndAcceptPermissionsQuery : IRequest<Result>
    {
        public int OrganizationId { get; set; }
        public string UserId { get; set; }
    }

    public class GetUserBySignAndAcceptPermissionsQueryHandler : IRequestHandler<GetUserBySignAndAcceptPermissionsQuery, Result>
    {
        private readonly IApplicationDbContext _context;
        private readonly IIdentityService _identityService;
        private readonly IMapper _mapper;

        public GetUserBySignAndAcceptPermissionsQueryHandler(IApplicationDbContext context, IIdentityService identityService, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        ...
        public async Task<Result> Handle(GetUserBySignAndAcceptPermissionsQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var permissionName = Regex.Replace(PowerUsersPermissions.SignAndAcceptContract.ToString(), "([A-Z])", " $1").Trim();
