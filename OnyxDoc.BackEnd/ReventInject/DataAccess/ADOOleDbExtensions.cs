...
namespace ReventInject.DataAccess
{
    public class ADOOleDbExtensions
    {
        ...
        public ADOOleDbExtensions(string _conStr, ILogger logger)
        {
            conStr = _conStr;
            _logger = logger;
        }

        public ADOOleDbExtensions(string _conStr, bool _InvokeTxn, ILogger logger)
        {
            conStr = _conStr;
            ...
            using (OleDbConnection conn = new OleDbConnection(conStr))
            {
                using (DataTable dt = new DataTable())
                {
                    ...
                    catch (OleDbException ex)
                    {
                        // handle error
                        result = new List<T>();
                        _logger.WriteErrorToEventLog((Exception)ex);
                    }
                    catch (Exception ex)
                    {
                        // handle error
                        _logger.WriteErrorToEventLog(ex);
                    }
                    finally
                    {
                        conn.Close();
