...
{
    public class DeleteCurrencyCommand : AuthToken, IRequest<Result>
    {
    ...
    public class DeleteCurrencyCommandHandler : IRequestHandler<DeleteCurrencyCommand, Result>
    {
        ...
        public DeleteCurrencyCommandHandler(IApplicationDbContext context, IAuthService authService)
        {
            _context = context;
        ...
        public async Task<Result> Handle(DeleteCurrencyCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                return Result.Success("Currency deleted successfully", entity);
            }
            catch (Exception ex)
            {
                return Result.Failure($"Delete currency failed. Error: { ex?.Message + Environment.NewLine + ex?.InnerException?.Message}");
            }
